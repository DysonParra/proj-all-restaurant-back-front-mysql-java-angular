/*
 * @fileoverview    {ChefRest} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ChefRest}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public class ChefRest {
    ...
    public CollectionModel<EntityModel<ChefDTO>> getAllEntities() {
        ...
        try {
            ...
            CollectionModel<EntityModel<ChefDTO>> entitiesCollection = new CollectionModel<>(entities);
            entitiesCollection.add(linkTo(methodOn(ChefRest.class).getAllEntities()).withSelfRel());
    ...
    public ResponseEntity<List<ChefDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/Chef/{query}/pages")
    public ResponseEntity<List<ChefDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Chef/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
